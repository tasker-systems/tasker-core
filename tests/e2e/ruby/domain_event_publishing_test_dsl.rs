//! # End-to-End Domain Event Publishing Integration Test
//!
...
/// Test successful domain event publishing workflow (Ruby Worker)
///
/// Validates:
...
/// - **TAS-65**: Verifies FFI channel dispatches for Ruby handler integration
///
/// **TAS-73 Note**: Skipped in cluster mode - metrics non-deterministic across instances.
#[tokio::test]
#[cfg_attr(feature = "test-cluster", ignore)]
async fn test_domain_event_publishing_success_dsl() -> Result<()> {
    ...
    // Use Ruby worker (port 8082) for event stats verification
    ...
    let customer_id = "ruby_customer_123";
    let amount = 99.99;

    println!("\n🎯 Creating domain event publishing task (Ruby worker)...");
    ...
    assert!(
        step_names.contains(&"domain_events_validate_order_dsl".to_string()),
        "Should have domain_events_validate_order step"
    );
    assert!(
        step_names.contains(&"domain_events_process_payment_dsl".to_string()),
        "Should have domain_events_process_payment step"
    );
    assert!(
        step_names.contains(&"domain_events_update_inventory_dsl".to_string()),
        "Should have domain_events_update_inventory step"
    );
    assert!(
        step_names.contains(&"domain_events_send_notification_dsl".to_string()),
        "Should have domain_events_send_notification step"
    );
...
#[tokio::test]
#[cfg_attr(feature = "test-cluster", ignore)]
async fn test_domain_event_publishing_concurrent_dsl() -> Result<()> {
    println!("🚀 Starting Domain Event Publishing Concurrent Test");
    println!("   Testing: 3 concurrent tasks with domain events");

    // Use Ruby worker (port 8082) for Ruby handler execution
    let manager = IntegrationTestManager::setup_ruby_worker().await?;
    ...
    for i in 0..task_count {
        let order_id = Uuid::new_v4().to_string();
        let customer_id = format!("customer_{}", i);
...
/// Test domain event workflow metrics availability (Ruby Worker)
///
/// Validates:
...
#[tokio::test]
async fn test_ruby_domain_event_metrics_availability_dsl() -> Result<()> {
    println!("🚀 Starting Domain Event Metrics Test (Ruby Worker)");

    // Use Ruby worker (port 8082) for Ruby-specific metrics
    let manager = IntegrationTestManager::setup_ruby_worker().await?;
