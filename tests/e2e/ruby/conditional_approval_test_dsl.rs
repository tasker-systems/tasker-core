...
#[tokio::test]
async fn test_small_amount_auto_approval_dsl() -> Result<()> {
    ...
    verify_steps_created(
        &steps,
        &[
            "validate_request_dsl",
            "routing_decision_dsl",
            "auto_approve_dsl",
            "finalize_approval_dsl",
        ],
    );
    ...
    let step_names: Vec<String> = steps.iter().map(|s| s.name.clone()).collect();
    assert!(
        !step_names.contains(&"manager_approval_dsl".to_string()),
        "Manager approval should NOT be created for small amounts"
    );
    assert!(
        !step_names.contains(&"finance_review_dsl".to_string()),
        "Finance review should NOT be created for small amounts"
    );

    // Verify decision step is complete
    verify_decision_step(&steps, "routing_decision_dsl");
...
#[tokio::test]
async fn test_medium_amount_manager_approval_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    let step_names: Vec<String> = steps.iter().map(|s| s.name.clone()).collect();
    assert!(
        !step_names.contains(&"auto_approve_dsl".to_string()),
...
#[tokio::test]
async fn test_large_amount_dual_approval_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    let manager_step = steps
        .iter()
        .find(|s| s.name == "manager_approval_dsl")
    ...
    let finance_step = steps
        .iter()
        .find(|s| s.name == "finance_review_dsl")
...
#[tokio::test]
async fn test_decision_point_step_dependency_structure_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Test that dynamically created steps have correct dependencies
    let task_request = create_approval_request_dsl(7500.0, "dave@example.com", "Software licenses");
...
#[tokio::test]
async fn test_boundary_conditions_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
...
#[tokio::test]
async fn test_boundary_large_threshold_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
...
#[tokio::test]
async fn test_very_small_amount_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
