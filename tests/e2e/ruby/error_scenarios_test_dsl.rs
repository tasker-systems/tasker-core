//! Ruby DSL Error Scenario E2E Tests
//!
...
/// - Fast execution (< 3s)
#[tokio::test]
async fn test_success_scenario_dsl() -> Result<()> {
...
#[tokio::test]
async fn test_permanent_failure_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with permanent-error-only template
    let task_request = create_task_request(
        "test_errors_dsl",
        "permanent_error_only_dsl",
...
#[tokio::test]
async fn test_retryable_failure_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with retryable-error-only template
    let task_request = create_task_request(
        "test_errors_dsl",
        "retryable_error_only_dsl",
...
#[tokio::test]
async fn test_mixed_workflow_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
