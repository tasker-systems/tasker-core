...
fn create_csv_processing_request_dsl(
    ...
    create_task_request(
        "csv_processing_dsl",
        "csv_product_inventory_analyzer_ruby_dsl",
        json!({
...
#[tokio::test]
async fn test_csv_batch_processing_with_ruby_handlers_dsl() -> Result<()> {
    ...
    assert!(
        step_names.contains(&"aggregate_csv_results_dsl".to_string()),
    ...
    let aggregate_step = steps
        .iter()
        .find(|s| s.name == "aggregate_csv_results_dsl")
...
#[tokio::test]
async fn test_no_batches_scenario_dsl() -> Result<()> {
