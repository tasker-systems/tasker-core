...
#[tokio::test]
async fn test_payments_namespace_refund_dsl() -> Result<()> {
    ...
    assert!(
        step_names.contains(&"validate_payment_eligibility_dsl".to_string()),
        "Should have validate_payment_eligibility step"
    );
    assert!(
        step_names.contains(&"process_gateway_refund_dsl".to_string()),
        "Should have process_gateway_refund step"
    );
    assert!(
        step_names.contains(&"update_payment_records_dsl".to_string()),
        "Should have update_payment_records step"
    );
    assert!(
        step_names.contains(&"notify_customer_dsl".to_string()),
...
#[tokio::test]
async fn test_customer_success_namespace_refund_dsl() -> Result<()> {
    ...
    let step_names: Vec<String> = steps.iter().map(|s| s.name.clone()).collect();
    assert!(
        step_names.contains(&"validate_refund_request_dsl".to_string()),
        "Should have validate_refund_request step"
    );
    assert!(
        step_names.contains(&"check_refund_policy_dsl".to_string()),
        "Should have check_refund_policy step"
    );
    assert!(
        step_names.contains(&"get_manager_approval_dsl".to_string()),
        "Should have get_manager_approval step"
    );
    assert!(
        step_names.contains(&"execute_refund_workflow_dsl".to_string()),
        "Should have execute_refund_workflow step (cross-namespace coordination)"
    );
    assert!(
        step_names.contains(&"update_ticket_status_dsl".to_string()),
...
#[tokio::test]
async fn test_concurrent_namespace_isolation_dsl() -> Result<()> {
