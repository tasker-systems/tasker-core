//! Python DSL Linear Workflow E2E Tests
//!
...
#[tokio::test]
async fn test_python_linear_workflow_standard_dsl() -> Result<()> {
    ...
    println!(
        "✅ Python DSL linear workflow completed with {} steps",
        steps.len()
    );
    Ok(())
}
...
/// - Step 4: 330 / 2 = 165
#[tokio::test]
async fn test_python_linear_workflow_different_input_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    println!("✅ Python DSL linear workflow with input 10 completed successfully");
    Ok(())
}
...
#[tokio::test]
async fn test_python_linear_workflow_ordering_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    assert!(
        step_names.contains(&"linear_step_1_dsl_py"),
        "Should have linear_step_1_py"
    );
    assert!(
        step_names.contains(&"linear_step_2_dsl_py"),
        "Should have linear_step_2_py"
    );
    assert!(
        step_names.contains(&"linear_step_3_dsl_py"),
        "Should have linear_step_3_py"
    );
    assert!(
        step_names.contains(&"linear_step_4_dsl_py"),
        "Should have linear_step_4_py"
    );

    println!("✅ Python DSL linear workflow steps executed in correct order");
    Ok(())
}
