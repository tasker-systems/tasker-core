//! Python DSL Diamond Workflow E2E Tests
//!
//! These tests validate diamond (parallel branches with convergence) workflow
//! execution with Python handlers through the orchestration API.
//!
...
/// Test diamond workflow with Python handlers using standard input
///
/// Validates:
...
#[tokio::test]
async fn test_python_diamond_workflow_standard_dsl() -> Result<()> {
    ...
    println!(
        "✅ Python DSL diamond workflow completed with {} steps",
        steps.len()
    );
    Ok(())
}
...
#[tokio::test]
async fn test_python_diamond_workflow_parallel_branches_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    assert!(
        step_names.contains(&"diamond_start_dsl_py"),
        "Should have diamond_start_py"
    );
    assert!(
        step_names.contains(&"diamond_branch_b_dsl_py"),
        "Should have diamond_branch_b_py"
    );
    assert!(
        step_names.contains(&"diamond_branch_c_dsl_py"),
        "Should have diamond_branch_c_py"
    );
    assert!(
        step_names.contains(&"diamond_end_dsl_py"),
        "Should have diamond_end_py"
    );
    ...
    println!("✅ Python DSL diamond workflow parallel branches executed correctly");
    Ok(())
}
...
/// - End: (41 + 32) / 2 = 36.5
#[tokio::test]
async fn test_python_diamond_workflow_different_input_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    println!("✅ Python DSL diamond workflow with input 4 completed successfully");
    Ok(())
}
