//! # End-to-End Domain Event Publishing Integration Test (Python)
//!
...
fn create_domain_event_task_request_dsl(
    order_id: &str,
    customer_id: &str,
    amount: f64,
    ...
    create_task_request(
        "domain_events_dsl_py",
        "domain_event_publishing_dsl_py",
        json!({
            "order_id": order_id,
...
/// Test successful domain event publishing workflow (Python Worker)
///
/// Validates:
...
async fn test_python_domain_event_publishing_success_dsl() -> Result<()> {
    ...
    println!("   Template: domain_event_publishing_py");
    println!("   Namespace: domain_events_py");
    println!("   Events: order.validated, payment.processed, inventory.updated, notification.sent");

    // Use Python worker (port 8083) for event stats verification
    let manager = IntegrationTestManager::setup_python_worker().await?;
    ...
    let worker_client = manager.worker_client.as_ref().ok_or_else(|| {
        anyhow::anyhow!(
            "Worker client required for domain event verification. \
             Set TASKER_TEST_PYTHON_WORKER_URL or ensure worker is running."
        )
    })?;
    ...
    let customer_id = "python_customer_123";
    let amount = 99.99;

    println!("\n🎯 Creating domain event publishing task (Python worker)...");
    println!("   Order ID: {}", order_id);
    println!("   Customer ID: {}", customer_id);
    println!("   Amount: ${:.2}", amount);

    let task_request = create_domain_event_task_request_dsl(&order_id, customer_id, amount, false);
    ...
    println!("\n⏱️  Monitoring domain event publishing workflow...");
    ...
    assert!(
        step_names.contains(&"domain_events_py_validate_order_dsl".to_string()),
        "Should have domain_events_py_validate_order step"
    );
    assert!(
        step_names.contains(&"domain_events_py_process_payment_dsl".to_string()),
        "Should have domain_events_py_process_payment step"
    );
    assert!(
        step_names.contains(&"domain_events_py_update_inventory_dsl".to_string()),
        "Should have domain_events_py_update_inventory step"
    );
    assert!(
        step_names.contains(&"domain_events_py_send_notification_dsl".to_string()),
        "Should have domain_events_py_send_notification step"
    );
    ...
    let ffi_dispatches = stats_after.in_process_bus.ffi_channel_dispatches
        - stats_before.in_process_bus.ffi_channel_dispatches;
    ...
    println!("   FFI channel dispatches: {}", ffi_dispatches);
    ...
    // TAS-65: Verify FFI channel dispatches for Python handler integration
    // Fast events should be dispatched to FFI subscribers (Python handlers)
    // Note: ffi_dispatches depends on Python handlers being registered to receive events
    println!(
        "   Note: FFI channel received {} dispatches (Python handler events)",
        ffi_dispatches
    );

    println!("\n✅ Python domain event publishing test passed!");
    ...
    println!("     • notification.sent (fast, success condition)");

    Ok(())
}

/// Test domain event publishing with multiple concurrent tasks
///
/// Validates:
...
#[tokio::test]
#[cfg_attr(feature = "test-cluster", ignore)]
async fn test_python_domain_event_publishing_concurrent_dsl() -> Result<()> {
    println!("🚀 Starting Domain Event Publishing Concurrent Test (Python)");
    println!("   Testing: 3 concurrent tasks with domain events");

    // Use Python worker (port 8083) for Python handler execution
    let manager = IntegrationTestManager::setup_python_worker().await?;
    ...
    for i in 0..task_count {
        let order_id = Uuid::new_v4().to_string();
        let customer_id = format!("python_customer_{}", i);

        let task_request = create_domain_event_task_request_dsl(
            &order_id,
            &customer_id,
            100.0 + (i as f64),
            false,
    ...
    println!("✅ Concurrent domain event publishing test passed!");
...
#[tokio::test]
async fn test_python_domain_event_metrics_availability_dsl() -> Result<()> {
    println!("🚀 Starting Domain Event Metrics Test (Python Worker)");

    // Use Python worker (port 8083) for Python-specific metrics
    let manager = IntegrationTestManager::setup_python_worker().await?;
