...
//! Python DSL Batch Processing E2E Tests
//!
...
fn create_csv_processing_request_dsl(
    csv_file_path: &str,
    analysis_mode: &str,
) -> tasker_shared::models::core::task_request::TaskRequest {
    create_task_request(
        "csv_processing_dsl_py",
        "csv_product_inventory_analyzer_dsl_py",
        json!({
...
#[tokio::test]
async fn test_python_csv_batch_processing_dsl() -> Result<()> {
    ...
    let task_request = create_csv_processing_request_dsl(&csv_file_path, "inventory");
    ...
    assert!(
        step_names.contains(&"aggregate_csv_results_dsl_py".to_string()),
    ...
    let aggregate_step = steps
        .iter()
        .find(|s| s.name == "aggregate_csv_results_dsl_py")
...
#[tokio::test]
async fn test_python_no_batches_scenario_dsl() -> Result<()> {
    ...
    let task_request = create_csv_processing_request_dsl(&empty_csv_path, "inventory");
...
#[tokio::test]
async fn test_python_batch_processing_scaling_dsl() -> Result<()> {
    ...
    let task_request = create_csv_processing_request_dsl(&csv_file_path, "pricing");
    ...
    let analyzer_step = steps
        .iter()
        .find(|s| s.name == "analyze_csv_dsl_py")
