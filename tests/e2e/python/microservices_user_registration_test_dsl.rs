//! # End-to-End Python Microservices User Registration Workflow Integration Test
//!
//! This integration test validates the Blog Post 03 microservices coordination pattern with Python handlers:
...
//! 4. Validating YAML configuration from tests/fixtures/task_templates/python/microservices_user_registration_py.yaml
//!
//! Prerequisites:
...
//!
//! TAS-91: Blog Post 03 - Python implementation
//!
...
/// Helper function to create user registration task request for Python
///
/// Creates a task request for the Python microservices workflow template.
/// Uses language-agnostic parameters that work regardless of handler implementation.
fn create_user_registration_request_dsl_py(
    user_info: serde_json::Value,
) -> tasker_shared::models::core::task_request::TaskRequest {
    create_task_request(
        "microservices_dsl_py",
        "user_registration_dsl_py",
        user_info,
    )
}

/// Test successful user registration workflow (Python)
///
/// Validates:
...
#[tokio::test]
async fn test_successful_user_registration_py_dsl() -> Result<()> {
    println!("🚀 Starting Python Microservices User Registration Test");
    println!("   Workflow: User creation → parallel billing/preferences → welcome → activation");
    println!("   Template: user_registration_py");
    println!("   Namespace: microservices_py");
    ...
    println!("\n🎯 Creating Python user registration task...");
    println!("   Email: {}", user_email);
    println!("   Plan: pro");
    let task_request = create_user_registration_request_dsl_py(user_info);
    ...
    println!("✅ Python microservices user registration completed successfully!");
