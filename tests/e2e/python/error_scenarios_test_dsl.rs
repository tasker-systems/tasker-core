//! Python DSL Error Scenario E2E Tests
//!
...
/// - Fast execution (< 3s)
#[tokio::test]
async fn test_python_success_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with Python success-only template
    let task_request = create_task_request(
        "test_scenarios_dsl_py",
        "success_only_dsl_py",
    ...
    println!("✅ Python DSL success scenario completed successfully");
    Ok(())
}
...
#[tokio::test]
async fn test_python_permanent_failure_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with Python permanent-error-only template
    let task_request = create_task_request(
        "test_scenarios_dsl_py",
        "permanent_error_only_dsl_py",
    ...
    println!(
        "✅ Python DSL permanent failure completed in {:?} (no retries)",
        elapsed
    );
    Ok(())
}
...
#[tokio::test]
async fn test_python_retryable_failure_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with Python retryable-error-only template
    let task_request = create_task_request(
        "test_scenarios_dsl_py",
        "retryable_error_only_dsl_py",
    ...
    println!(
        "✅ Python DSL retryable failure completed in {:?} (with retries)",
        elapsed
    );
    Ok(())
}
