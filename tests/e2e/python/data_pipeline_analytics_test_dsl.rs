...
fn create_data_pipeline_request_dsl_py(
    pipeline_id: &str,
    date_range: serde_json::Value,
) -> tasker_shared::models::core::task_request::TaskRequest {
    create_task_request(
        "data_pipeline_dsl_py",
        "analytics_pipeline_dsl_py",
        json!({
...
#[tokio::test]
async fn test_successful_analytics_pipeline_py_dsl() -> Result<()> {
    ...
    let task_request = create_data_pipeline_request_dsl_py(&pipeline_id, date_range);
    ...
    assert!(
        step_names.contains(&"extract_sales_data_dsl_py".to_string()),
        "Should have extract_sales_data step"
    );
    assert!(
        step_names.contains(&"extract_inventory_data_dsl_py".to_string()),
        "Should have extract_inventory_data step"
    );
    assert!(
        step_names.contains(&"extract_customer_data_dsl_py".to_string()),
        "Should have extract_customer_data step"
    );

    // Transform phase (3 sequential steps)
    assert!(
        step_names.contains(&"transform_sales_dsl_py".to_string()),
        "Should have transform_sales step"
    );
    assert!(
        step_names.contains(&"transform_inventory_dsl_py".to_string()),
        "Should have transform_inventory step"
    );
    assert!(
        step_names.contains(&"transform_customers_dsl_py".to_string()),
        "Should have transform_customers step"
    );

    // Aggregate phase (1 step - DAG convergence)
    assert!(
        step_names.contains(&"aggregate_metrics_dsl_py".to_string()),
        "Should have aggregate_metrics step (DAG convergence)"
    );

    // Insights phase (1 step)
    assert!(
        step_names.contains(&"generate_insights_dsl_py".to_string()),
