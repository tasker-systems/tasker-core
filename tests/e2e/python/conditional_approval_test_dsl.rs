...
//! Python DSL Conditional Approval E2E Tests
//!
...
fn create_approval_request_dsl(
    amount: f64,
    requester: &str,
    purpose: &str,
) -> tasker_shared::models::core::task_request::TaskRequest {
    create_task_request(
        "conditional_approval_dsl_py",
        "approval_routing_dsl_py",
        json!({
            "amount": amount,
            "requester": requester,
            "purpose": purpose
...
#[tokio::test]
async fn test_python_small_amount_auto_approval_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Small amount: $500 should trigger auto-approval path
    let task_request = create_approval_request_dsl(500.0, "alice@example.com", "Office supplies");
    ...
    verify_steps_created(
        &steps,
        &[
            "validate_request_dsl_py",
            "routing_decision_dsl_py",
            "auto_approve_dsl_py",
            "finalize_approval_dsl_py",
        ],
    );
    ...
    let step_names: Vec<String> = steps.iter().map(|s| s.name.clone()).collect();
    assert!(
        !step_names.contains(&"manager_approval_dsl_py".to_string()),
        "Manager approval should NOT be created for small amounts"
    );
    assert!(
        !step_names.contains(&"finance_review_dsl_py".to_string()),
        "Finance review should NOT be created for small amounts"
    );

    // Verify decision step is complete
    verify_decision_step(&steps, "routing_decision_dsl_py");
    ...
    println!("✅ Python DSL small amount auto-approval test passed");
    Ok(())
}
...
/// - Task completes successfully with 4 steps
#[tokio::test]
async fn test_python_medium_amount_manager_approval_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Medium amount: $2,500 should trigger manager-only approval path
    let task_request = create_approval_request_dsl(2500.0, "bob@example.com", "New equipment");
    ...
    let step_names: Vec<String> = steps.iter().map(|s| s.name.clone()).collect();
    assert!(
        !step_names.contains(&"auto_approve_dsl_py".to_string()),
    ...
    println!("✅ Python DSL medium amount manager-approval test passed");
    Ok(())
}
...
#[tokio::test]
async fn test_python_large_amount_dual_approval_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Large amount: $10,000 should trigger dual-approval path
    let task_request =
        create_approval_request_dsl(10000.0, "charlie@example.com", "Capital investment");
    ...
    let manager_step = steps
        .iter()
        .find(|s| s.name == "manager_approval_dsl_py")
    ...
    let finance_step = steps
        .iter()
        .find(|s| s.name == "finance_review_dsl_py")
    ...
    println!("✅ Python DSL large amount dual-approval test passed");
    Ok(())
}
...
#[tokio::test]
async fn test_python_boundary_small_threshold_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Test exactly at $1,000 threshold (should trigger manager approval)
    let task_request = create_approval_request_dsl(1000.0, "eve@example.com", "Threshold test");
    ...
    println!("✅ Python DSL boundary $1,000 threshold test passed");
    Ok(())
}
...
#[tokio::test]
async fn test_python_boundary_large_threshold_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Test exactly at $5,000 threshold (should trigger dual approval)
    let task_request =
        create_approval_request_dsl(5000.0, "frank@example.com", "Large threshold test");
    ...
    println!("✅ Python DSL boundary $5,000 threshold test passed");
    Ok(())
}
...
#[tokio::test]
async fn test_python_very_small_amount_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Test very small amount: $0.01
    let task_request = create_approval_request_dsl(0.01, "grace@example.com", "Petty cash");
    ...
    println!("✅ Python DSL very small amount test passed");
    Ok(())
}
