//! TAS-294 DSL: Resolver Chain E2E Tests for Python
//!
//! These tests validate the resolver chain features with Python handlers:
...
/// Test method dispatch with Python handlers
///
/// Validates:
...
#[tokio::test]
async fn test_python_method_dispatch_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task that uses method dispatch
    let task_request = create_task_request(
        "resolver_tests_dsl_py",
        "method_dispatch_dsl_py",
        json!({
            "data": {
                "amount": 100.0,
                "reason": "test_refund"
    ...
    assert!(
        step_names.contains(&"validate_step_dsl_py"),
        "Should have validate_step_py"
    );
    assert!(
        step_names.contains(&"process_step_dsl_py"),
        "Should have process_step_py"
    );
    assert!(
        step_names.contains(&"refund_step_dsl_py"),
        "Should have refund_step_py"
    );
    assert!(
        step_names.contains(&"default_call_step_dsl_py"),
        "Should have default_call_step_py"
    );
    ...
    println!("✅ Python DSL method dispatch test completed - all 4 methods invoked successfully");
    Ok(())
}
...
/// Test that existing Python templates without method/resolver fields still work
///
/// Validates:
...
#[tokio::test]
async fn test_python_backward_compatibility_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Use existing linear workflow template (no method/resolver fields)
    let task_request = create_task_request(
        "linear_workflow_dsl_py",
        "mathematical_sequence_dsl_py",
        json!({ "even_number": 6 }),
    );
    ...
    println!("✅ Python DSL backward compatibility verified - existing templates work");
    Ok(())
}
