//! TypeScript DSL Conditional Approval E2E Tests
//!
...
#[tokio::test]
async fn test_typescript_small_amount_auto_approval_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with small amount ($500)
    let task_request = create_task_request(
        "conditional_approval_dsl_ts",
        "approval_routing_dsl_ts",
        json!({
            "amount": 500.0,
            "requester": "test_user",
    ...
    assert!(
        step_names.contains(&"validate_request_dsl_ts"),
        "Should have validate_request_ts"
    );
    assert!(
        step_names.contains(&"routing_decision_dsl_ts"),
        "Should have routing_decision_ts"
    );
    assert!(
        step_names.contains(&"auto_approve_dsl_ts"),
        "Should have auto_approve_ts"
    );
    assert!(
        step_names.contains(&"finalize_approval_dsl_ts"),
...
/// - Task completes with 4 steps
#[tokio::test]
async fn test_typescript_medium_amount_manager_approval_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    let step_names: Vec<&str> = steps.iter().map(|s| s.name.as_str()).collect();
    assert!(
        step_names.contains(&"manager_approval_dsl_ts"),
...
#[tokio::test]
async fn test_typescript_large_amount_dual_approval_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    assert!(
        step_names.contains(&"finance_review_dsl_ts"),
...
#[tokio::test]
async fn test_typescript_boundary_small_threshold_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
...
#[tokio::test]
async fn test_typescript_boundary_large_threshold_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
