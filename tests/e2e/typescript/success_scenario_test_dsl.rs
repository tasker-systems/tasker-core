//! TypeScript DSL Success Scenario E2E Tests
//!
...
#[tokio::test]
async fn test_typescript_success_no_input_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task without optional message
    let task_request =
        create_task_request("test_scenarios_dsl_ts", "success_only_dsl_ts", json!({}));
