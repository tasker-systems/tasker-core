//! TypeScript DSL Diamond Workflow E2E Tests
//!
...
#[tokio::test]
async fn test_typescript_diamond_workflow_standard_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with input 6
    // Expected: 6² = 36, branch_b = 36+25 = 61, branch_c = 36*2 = 72, end = (61+72)/2 = 66.5
    let task_request = create_task_request(
        "diamond_workflow_dsl_ts",
        "parallel_computation_dsl_ts",
        json!({ "even_number": 6 }),
    );
...
#[tokio::test]
async fn test_typescript_diamond_workflow_parallel_branches_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    assert!(
        step_names.contains(&"diamond_start_dsl_ts"),
        "Should have diamond_start_ts"
    );
    assert!(
        step_names.contains(&"diamond_branch_b_dsl_ts"),
        "Should have diamond_branch_b_ts"
    );
    assert!(
        step_names.contains(&"diamond_branch_c_dsl_ts"),
        "Should have diamond_branch_c_ts"
    );
    assert!(
        step_names.contains(&"diamond_end_dsl_ts"),
...
/// - End: (41 + 32) / 2 = 36.5
#[tokio::test]
async fn test_typescript_diamond_workflow_different_input_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
