//! TypeScript DSL Error Scenario E2E Tests
//!
...
/// - Fast execution (< 3s)
#[tokio::test]
async fn test_typescript_success_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with TypeScript success-only template
    let task_request = create_task_request(
        "test_scenarios_dsl_ts",
        "success_only_dsl_ts",
...
#[tokio::test]
async fn test_typescript_permanent_failure_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with TypeScript permanent-error-only template
    let task_request = create_task_request(
        "test_scenarios_dsl_ts",
        "permanent_error_only_dsl_ts",
...
#[tokio::test]
async fn test_typescript_retryable_failure_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with TypeScript retryable-error-only template
    let task_request = create_task_request(
        "test_scenarios_dsl_ts",
        "retryable_error_only_dsl_ts",
