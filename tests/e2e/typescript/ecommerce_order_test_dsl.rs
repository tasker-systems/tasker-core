...
fn create_ecommerce_order_request_dsl_ts(
    ...
    create_task_request(
        "ecommerce_dsl_ts",
        "ecommerce_order_processing_dsl_ts",
        json!({
...
#[tokio::test]
async fn test_successful_order_processing_ts_dsl() -> Result<()> {
    ...
    let task_request =
        create_ecommerce_order_request_dsl_ts(cart_items, customer_info, payment_info);
    ...
    assert!(
        step_names.contains(&"validate_cart_dsl_ts".to_string()),
        "Should have validate_cart step"
    );
    assert!(
        step_names.contains(&"process_payment_dsl_ts".to_string()),
        "Should have process_payment step"
    );
    assert!(
        step_names.contains(&"update_inventory_dsl_ts".to_string()),
        "Should have update_inventory step"
    );
    assert!(
        step_names.contains(&"create_order_dsl_ts".to_string()),
        "Should have create_order step"
    );
    assert!(
        step_names.contains(&"send_confirmation_dsl_ts".to_string()),
