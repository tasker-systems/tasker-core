//! TypeScript DSL Batch Processing E2E Tests
//!
...
#[tokio::test]
async fn test_typescript_csv_batch_processing_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create batch processing task
    // The handler simulates 1000 rows with batch size 200 = 5 workers
    let task_request = create_task_request(
        "csv_processing_dsl_ts",
        "csv_product_inventory_analyzer_dsl_ts",
        json!({
    ...
    assert!(
        step_names.contains(&"analyze_csv_dsl_ts"),
        "Should have analyze_csv_ts (batchable step)"
    );
    assert!(
        step_names.contains(&"aggregate_csv_results_dsl_ts"),
...
#[tokio::test]
async fn test_typescript_no_batches_scenario_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
