//! TypeScript DSL Linear Workflow E2E Tests
//!
//! These tests validate sequential workflow execution with TypeScript handlers
...
/// Test linear workflow with TypeScript handlers using standard input
///
/// Validates:
...
#[tokio::test]
async fn test_typescript_linear_workflow_standard_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task with input 6
    // Expected: 6² = 36, 36+10 = 46, 46*3 = 138, 138/2 = 69
    let task_request = create_task_request(
        "linear_workflow_dsl_ts",
        "mathematical_sequence_dsl_ts",
        json!({ "even_number": 6 }),
    );
    ...
    println!(
        "✅ TypeScript linear workflow completed with {} steps",
        steps.len()
    );
    Ok(())
}

/// Test linear workflow with TypeScript handlers using different input
///
...
/// - Step 4: 330 / 2 = 165
#[tokio::test]
async fn test_typescript_linear_workflow_different_input_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    println!("✅ TypeScript linear workflow with input 10 completed successfully");
    Ok(())
}
...
#[tokio::test]
async fn test_typescript_linear_workflow_ordering_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    println!("✅ TypeScript linear workflow steps executed in correct order");
    Ok(())
}
