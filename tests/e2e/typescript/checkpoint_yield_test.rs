...
//! TAS-125: TypeScript Checkpoint Yield E2E Tests
//!
...
fn create_checkpoint_yield_request(
    ...
    create_task_request("ts_e2e_checkpoint", "checkpoint_yield_test_ts", context)
}
...
#[tokio::test]
async fn test_typescript_checkpoint_yield_happy_path() -> Result<()> {
    ...
    assert!(
        step_names.contains(&"analyze_items_ts".to_string()),
        "Should have analyze_items_ts step"
    );
    assert!(
        step_names.contains(&"aggregate_results_ts".to_string()),
    ...
    let aggregate_step = steps
        .iter()
        .find(|s| s.name == "aggregate_results_ts")
...
#[tokio::test]
async fn test_typescript_checkpoint_yield_transient_failure_resume() -> Result<()> {
    ...
    let batch_worker = steps
        .iter()
        .find(|s| s.name.starts_with("checkpoint_yield_batch_ts"))
...
#[tokio::test]
async fn test_typescript_checkpoint_yield_permanent_failure() -> Result<()> {
