//! TAS-294 DSL: Resolver Chain E2E Tests for TypeScript
//!
...
#[tokio::test]
async fn test_typescript_method_dispatch_validate_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    // Create task that uses method dispatch to call validate()
    let task_request = create_task_request(
        "resolver_tests_dsl_ts",
        "method_dispatch_dsl_ts",
        json!({
            "data": {
                "amount": 100.0,
                "reason": "test_refund"
...
#[tokio::test]
async fn test_typescript_method_dispatch_multiple_methods_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
...
#[tokio::test]
async fn test_typescript_resolver_hints_explicit_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;

    let task_request = create_task_request(
        "resolver_tests_dsl_ts",
        "resolver_hints_dsl_ts",
        json!({
...
#[tokio::test]
async fn test_typescript_resolver_hints_with_method_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
...
#[tokio::test]
async fn test_typescript_backward_compatibility_no_method_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
...
#[tokio::test]
async fn test_typescript_explicit_call_method_dsl() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
