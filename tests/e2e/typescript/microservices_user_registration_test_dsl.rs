//! # End-to-End TypeScript Microservices User Registration Workflow Integration Test
//!
//! This integration test validates the Blog Post 03 microservices coordination pattern with TypeScript handlers:
...
//! 4. Validating YAML configuration from tests/fixtures/task_templates/typescript/microservices_user_registration_ts.yaml
//!
//! Prerequisites:
...
//!
//! TAS-91: Blog Post 03 - TypeScript implementation
//!
//! NOTE: This test is language-agnostic and uses the tasker-client API. It does NOT reference
//! TypeScript DSL code or handlers directly - ensuring the system works correctly regardless of worker
...
/// Helper function to create user registration task request for TypeScript
///
/// Creates a task request for the TypeScript microservices workflow template.
/// Uses language-agnostic parameters that work regardless of handler implementation.
fn create_user_registration_request_dsl_ts(
    user_info: serde_json::Value,
) -> tasker_shared::models::core::task_request::TaskRequest {
    create_task_request(
        "microservices_dsl_ts",
        "user_registration_dsl_ts",
        user_info,
    )
}

/// Test successful user registration workflow (TypeScript)
///
/// Validates:
...
#[tokio::test]
async fn test_successful_user_registration_ts_dsl() -> Result<()> {
    println!("🚀 Starting TypeScript Microservices User Registration Test");
    println!("   Workflow: User creation → parallel billing/preferences → welcome → activation");
    println!("   Template: user_registration_ts");
    println!("   Namespace: microservices_ts");
    ...
    println!("\n🎯 Creating TypeScript user registration task...");
    println!("   Email: {}", user_email);
    println!("   Plan: pro");
    let task_request = create_user_registration_request_dsl_ts(user_info);
    ...
    assert!(
        step_names.contains(&"create_user_account_dsl_ts".to_string()),
        "Should have create_user_account step"
    );

    // Parallel phase (2 steps)
    assert!(
        step_names.contains(&"setup_billing_profile_dsl_ts".to_string()),
        "Should have setup_billing_profile step"
    );
    assert!(
        step_names.contains(&"initialize_preferences_dsl_ts".to_string()),
        "Should have initialize_preferences step"
    );

    // Coordination phase (1 step - convergence)
    assert!(
        step_names.contains(&"send_welcome_sequence_dsl_ts".to_string()),
        "Should have send_welcome_sequence step (convergence)"
    );

    // Completion phase (1 step - final)
    assert!(
        step_names.contains(&"update_user_status_dsl_ts".to_string()),
        "Should have update_user_status step (final)"
    );

    println!("✅ TypeScript microservices user registration completed successfully!");
