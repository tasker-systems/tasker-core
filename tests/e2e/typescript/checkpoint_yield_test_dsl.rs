...
//! TAS-294 DSL: TypeScript Checkpoint Yield E2E Tests
//!
...
//!
//! Workflow Pattern:
//! 1. analyze_items_ts (batchable): Create a single batch for checkpoint testing
//! 2. checkpoint_yield_batch_ts (batch_worker): Process items with checkpoint yields
//! 3. aggregate_results_ts (deferred_convergence): Aggregate final output
//!
//! Test Scenarios:
...
//!
//! Note: These tests require the TypeScript worker running on port 8084.
...
/// Create checkpoint yield test task request for TypeScript handlers.
fn create_checkpoint_yield_request_dsl(
    total_items: i32,
    ...
    create_task_request(
        "ts_e2e_checkpoint_dsl",
        "checkpoint_yield_test_dsl_ts",
        context,
    )
}

/// Test checkpoint yield happy path with TypeScript handlers.
///
/// Validates:
...
#[tokio::test]
async fn test_typescript_checkpoint_yield_happy_path_dsl() -> Result<()> {
    println!("🚀 Starting TAS-125 Checkpoint Yield Happy Path Test (TypeScript)");
    println!("   Total items: 100");
    println!("   Items per checkpoint: 25");
    println!("   Expected: 4 checkpoint yields, then success");
    println!("   Template: checkpoint_yield_test_ts");
    println!("   Namespace: ts_e2e_checkpoint");
    ...
    let task_request = create_checkpoint_yield_request_dsl(
        100, // total_items
    ...
    assert!(
        step_names.contains(&"analyze_items_dsl_ts".to_string()),
        "Should have analyze_items_ts step"
    );
    assert!(
        step_names.contains(&"aggregate_results_dsl_ts".to_string()),
        "Should have aggregate_results_ts step"
    );
    ...
    let aggregate_step = steps
        .iter()
        .find(|s| s.name == "aggregate_results_dsl_ts")
        .expect("Should have aggregate_results_ts step");
    ...
    println!("✅ TypeScript Batchable checkpointYield(): Working");
...
#[tokio::test]
async fn test_typescript_checkpoint_yield_transient_failure_resume_dsl() -> Result<()> {
    println!("🚀 Starting TAS-125 Checkpoint Yield Transient Failure Test (TypeScript)");
    println!("   Total items: 100");
    ...
    let batch_worker = steps
        .iter()
        .find(|s| s.name.starts_with("checkpoint_yield_batch_dsl_ts"))
...
#[tokio::test]
async fn test_typescript_checkpoint_yield_permanent_failure_dsl() -> Result<()> {
    println!("🚀 Starting TAS-125 Checkpoint Yield Permanent Failure Test (TypeScript)");
    println!("   Total items: 100");
