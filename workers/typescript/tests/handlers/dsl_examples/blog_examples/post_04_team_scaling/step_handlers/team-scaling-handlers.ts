...
export const CheckRefundPolicyDslHandler = defineHandler(
  ...
  {
    depends: { validationResult: 'validate_refund_request_dsl_ts' },
  ...
  async ({ validationResult, context }) => {
    ...
    const customerTier =
      (context.getDependencyField('validate_refund_request_dsl_ts', 'customer_tier') as string) ||
      'standard';
    const purchaseDateStr = context.getDependencyField(
      'validate_refund_request_dsl_ts',
...
export const GetManagerApprovalDslHandler = defineHandler(
  ...
  {
    depends: {
      policyResult: 'check_refund_policy_dsl_ts',
      validationResult: 'validate_refund_request_dsl_ts',
    },
  },
  async ({ policyResult, context }) => {
    ...
    const ticketId = context.getDependencyField(
      'validate_refund_request_dsl_ts',
      'ticket_id'
    ) as string;
...
export const ExecuteRefundWorkflowDslHandler = defineHandler(
  ...
  {
    depends: {
      approvalResult: 'get_manager_approval_dsl_ts',
      validationResult: 'validate_refund_request_dsl_ts',
    },
  },
...
export const UpdateTicketStatusDslHandler = defineHandler(
  ...
  {
    depends: {
      delegationResult: 'execute_refund_workflow_dsl_ts',
      validationResult: 'validate_refund_request_dsl_ts',
    },
    inputs: { refundAmount: 'refund_amount' },
  },
  async ({ delegationResult, context }) => {
    ...
    const delegatedTaskId = context.getDependencyField(
      'execute_refund_workflow_dsl_ts',
      'delegated_task_id'
    ) as string;
...
export const ProcessGatewayRefundDslHandler = defineHandler(
  ...
  { depends: { validationResult: 'validate_payment_eligibility_dsl_ts' } },
  async ({ validationResult, context }) => {
    ...
    const paymentId = context.getDependencyField(
      'validate_payment_eligibility_dsl_ts',
      'payment_id'
    ) as string;
    ...
    if (paymentId.includes('pay_test_gateway_error')) {
      return StepHandlerResult.failure(
        'Gateway refund failed: Gateway error',
        ErrorType.PERMANENT_ERROR,
        false
      );
    }
...
export const UpdatePaymentRecordsDslHandler = defineHandler(
  ...
  { depends: { refundResult: 'process_gateway_refund_dsl_ts' } },
  async ({ refundResult, context }) => {
    ...
    const paymentId = context.getDependencyField(
      'process_gateway_refund_dsl_ts',
      'payment_id'
    ) as string;
    const refundId = context.getDependencyField(
      'process_gateway_refund_dsl_ts',
      'refund_id'
    ) as string;
...
export const NotifyCustomerDslHandler = defineHandler(
  ...
  async ({ refundResult, customerEmail, context }) => {
    ...
    if (!/^[^@\s]+@[^@\s]+$/.test(email)) {
      return StepHandlerResult.failure(
        `Invalid customer email format: ${email}`,
        ErrorType.PERMANENT_ERROR,
        false
      );
    }
