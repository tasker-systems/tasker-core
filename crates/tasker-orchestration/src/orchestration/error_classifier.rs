...
use std::collections::HashMap;
use std::time::Duration;
use tasker_shared::errors::{ExecutionError, OrchestrationError, StepExecutionError};
use uuid::Uuid;
    ...
    fn classify_error(
        ...
        match error {
            ...
            OrchestrationError::ConfigurationError {
                config_source,
                reason,
            } => self.classify_configuration_error(config_source, reason, context),
            OrchestrationError::ValidationError { field, reason } => {
                self.classify_validation_error(field, reason, context)
            }
            OrchestrationError::ExecutionError { error } => {
                self.classify_execution_error(error, context)
            }
            _ => {
...
mod tests {
    ...
    fn test_configuration_error_classification() {
        ...
        let error = OrchestrationError::ConfigurationError {
            config_source: "yaml_file".to_string(),
