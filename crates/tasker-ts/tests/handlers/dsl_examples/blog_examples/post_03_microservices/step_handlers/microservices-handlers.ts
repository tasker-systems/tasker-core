...
/** Build the email body for the welcome sequence. Extracted to reduce handler complexity. */
function buildWelcomeEmailBody(
  template: { greeting: string; highlights: string[]; upgrade_prompt: string | null },
  plan: string,
  billing: BillingData,
  userId: string
): string {
  ...
  if (plan !== 'free' && billing.billing_id) {
    parts.push('', `Billing ID: ${billing.billing_id}`);
    parts.push(`Next billing date: ${billing.next_billing_date}`);
  }
...
export const CreateUserAccountDslHandler = defineHandler(
  ...
  async ({ userInfo }) => {
    ...
    if (!isValidEmail(info.email)) {
      return StepHandlerResult.failure(
        `Invalid email format: ${info.email}`,
        ErrorType.PERMANENT_ERROR,
        false
      );
    }
...
export const SetupBillingProfileDslHandler = defineHandler(
  ...
  { depends: { userData: 'create_user_account_dsl_ts' } },
  async ({ userData }) => {
...
export const SendWelcomeSequenceDslHandler = defineHandler(
  ...
  {
    depends: {
      userData: 'create_user_account_dsl_ts',
      billingData: 'setup_billing_profile_dsl_ts',
      preferencesData: 'initialize_preferences_dsl_ts',
    },
  },
  async ({ userData, billingData, preferencesData }) => {
    ...
    if (!user) missing.push('create_user_account_dsl_ts');
    if (!billing) missing.push('setup_billing_profile_dsl_ts');
    if (!preferences) missing.push('initialize_preferences_dsl_ts');
    ...
    if (prefs.email_notifications !== false) {
      ...
      messagesSent.push({
        ...
        body: buildWelcomeEmailBody(template, plan, validBilling, userId),
        sent_at: now,
      });
    }
...
export const UpdateUserStatusDslHandler = defineHandler(
  'MicroservicesDsl.StepHandlers.UpdateUserStatusDslHandler',
  {
    depends: {
      ...
      billingData: 'setup_billing_profile_dsl_ts',
      preferencesData: 'initialize_preferences_dsl_ts',
      welcomeData: 'send_welcome_sequence_dsl_ts',
    },
  },
  async ({ userData, billingData, preferencesData, welcomeData }) => {
    ...
    if (!billing) missing.push('setup_billing_profile_dsl_ts');
    if (!preferences) missing.push('initialize_preferences_dsl_ts');
    if (!welcome) missing.push('send_welcome_sequence_dsl_ts');
