...
# Builds the TypeScript FFI extension (C-FFI cdylib) for a target architecture.
#
# Usage:
#   ./scripts/ffi-build/build-typescript.sh [--target TARGET_TRIPLE]
#
# If --target is not specified, builds for the native architecture.
# Produces libtasker_ts-<target>.{so,dylib} in artifacts/<target>/typescript/
...
log_header "Building TypeScript FFI Extension"
...
DEST_NAME="libtasker_ts-${TARGET}.${EXT}"
