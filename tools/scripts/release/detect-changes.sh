...
#     SERVER_CORE_CHANGED=true|false   - tasker-orchestration, tasker-client, tasker-ctl, tasker-sdk, tasker-mcp
...
if changes_match '^crates/(tasker-pgmq|tasker-shared|tasker-worker)/'; then
    FFI_CORE_CHANGED=true
fi

# Server/client core: changes here only affect Rust crates, no FFI rebuild
SERVER_CORE_CHANGED=false
if changes_match '^crates/(tasker-orchestration|tasker-client|tasker-ctl|tasker-sdk|tasker-mcp)/'; then
...
if changes_match '^tools/scripts/(ffi-build/build-ruby\.sh|release/(build-ruby-gems\.sh|publish-ruby\.sh))'; then
    RUBY_INFRA_CHANGED=true
fi

PYTHON_INFRA_CHANGED=false
if changes_match '^tools/scripts/(ffi-build/build-python\.sh|release/publish-python\.sh)'; then
    PYTHON_INFRA_CHANGED=true
fi

TYPESCRIPT_INFRA_CHANGED=false
if changes_match '^tools/scripts/(ffi-build/build-typescript\.sh|release/publish-typescript\.sh)'; then
