...
collect_scripts() {
    local dirs="tools/scripts tools/cargo-make/scripts .github/scripts tools/bin docker/scripts"
    for dir in $dirs; do
        if [[ -d "$dir" ]]; then
...
while IFS= read -r makefile; do
    ...
    while IFS= read -r ref; do
        ...
        # Resolve ${SCRIPTS_DIR} to tools/cargo-make/scripts (relative to repo root)
        resolved=$(echo "$ref" | sed 's|\${SCRIPTS_DIR}|tools/cargo-make/scripts|g')

        # If path doesn't start with / or cargo-make, it's relative to the makefile dir
        if [[ "$resolved" != /* ]] && [[ "$resolved" != tools/cargo-make/* ]]; then
