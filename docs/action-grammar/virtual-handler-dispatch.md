...
This isn't a hypothetical — it's the logical extreme of the architecture described above. The `crates/tasker-example-rs` crate already demonstrates the pattern: a thin binary that bootstraps from `tasker-worker` and provides its own handler registry. A composition-only worker would be even thinner: **no handler registry**, because the `CompositionExecutor` is the handler.

### What Today's Rust Worker Does

The existing `crates/tasker-example-rs` bootstrap (`crates/tasker-example-rs/src/bootstrap.rs`) follows this sequence:
...
The composition-only worker follows the same pattern as `crates/tasker-example-rs` — a thin crate in the workspace:
