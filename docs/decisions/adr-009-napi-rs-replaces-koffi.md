...
The existing TypeScript worker (`crates/workers/typescript/`) has a multi-layer runtime abstraction:
...
release.yml: publish-typescript job
  ├── Download artifacts → crates/workers/typescript/native/
...
- mkdir -p crates/workers/typescript/native
- cp ffi-artifacts/typescript/libtasker_ts-x86_64-unknown-linux-gnu.so \
-    crates/workers/typescript/native/libtasker_ts-linux-x64.so
- cp ffi-artifacts/typescript/libtasker_ts-aarch64-apple-darwin.dylib \
-    crates/workers/typescript/native/libtasker_ts-darwin-arm64.dylib
+ # napi-rs .node files go at package root (loader expects them there)
+ cp ffi-artifacts/typescript/*.node crates/workers/typescript/
```
...
build-ffi-libraries.yml (matrix: linux-x64, darwin-arm64)
  ...
  └── Native: npx napi build --release --platform --target aarch64-apple-darwin
        → tasker-ts-napi.darwin-arm64.node
          ↓
release.yml: publish-typescript job
  ├── Download artifacts → cp *.node crates/workers/typescript/
  ├── bun install && bun run build
...
| `crates/workers/typescript/deno.json` | Delete | No dedicated Deno adapter |
...
1. Rename/replace `crates/workers/typescript/src-rust/` with napi-rs implementation
