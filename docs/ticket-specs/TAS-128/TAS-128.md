...
| **TAS-136** | Legacy schema cleanup (drop annotations, dependent_systems, bypass_steps) | Coordinate |
...
**TAS-136 Coordination**: When creating the flattened migration baseline, incorporate TAS-136's schema removals (dropped tables, columns) into the baseline rather than as separate DROP statements. This results in a clean schema without migration noise while maintaining clear ticket-level accountability.
...
| `tasker-pgmq` | internal | N/A | Uses standard pgmq API, compatible |
...
**Tables** (14 tables in `public` schema with `tasker_` prefix, 10 after TAS-136):
...
- ~~`tasker_dependent_systems`~~ - *(Removed by TAS-136)*
- ~~`tasker_annotation_types`~~ - *(Removed by TAS-136)*
- ~~`tasker_task_annotations`~~ - *(Removed by TAS-136)*
- `tasker_named_tasks_named_steps` - Task-step join table
- ~~`tasker_dependent_system_object_maps`~~ - *(Removed by TAS-136)*
- `tasker_workflow_step_edges` - Step dependency edges
- `tasker_workflow_step_result_audit` - Step result audit (TAS-62)
- `tasker_tasks_dlq` - Dead letter queue tracking (TAS-49)
...
| `tasker_tasks_dlq` | `tasks_dlq` |

**Note**: Tables `tasker_dependent_systems`, `tasker_annotation_types`, `tasker_task_annotations`, and `tasker_dependent_system_object_maps` are removed by TAS-136 and will not be present in the flattened schema.
