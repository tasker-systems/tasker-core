...
```
tasker-core/
├── Makefile.toml                    # Root orchestrator (extends base files)
├── .config/
│   └── nextest.toml                 # Nextest profiles (already complete)
├── cargo-make/
...
│       ├── run-migrations.sh        # SQLx migrations
│       ├── reset-db.sh              # Database reset
│       ├── sqlx-prepare.sh          # Update query cache
│       ├── setup-workers.sh         # Worker setup orchestration
│       ├── clean-workers.sh         # Worker cleanup
│       ├── check-services.sh        # Service health checks
│       └── setup-env.sh             # Environment setup (local)
├── tasker-pgmq/
│   └── Makefile.toml                # Library crate tasks
├── tasker-client/
│   └── Makefile.toml                # Library crate tasks
├── tasker-orchestration/
│   └── Makefile.toml                # Service crate tasks (includes run)
├── tasker-shared/
│   └── Makefile.toml                # Library crate tasks
├── tasker-worker/
│   └── Makefile.toml                # Service crate tasks (includes run)
├── workers/
│   ├── rust/
│   │   └── Makefile.toml            # Rust worker tasks (includes run)
│   ├── ruby/
│   │   └── Makefile.toml            # Extends base (update for consistency)
│   ├── python/
│   │   └── Makefile.toml            # Extends base (update for consistency)
│   └── typescript/
│       └── Makefile.toml            # Extends base, multi-runtime FFI
...
script = '''echo "✓ Rust worker has no setup requirements"'''
```
...
1. ✅ All actions use consistent naming (`check`, `test`, `fix`, `build`, `setup`)
2. ✅ All workers have consistent `setup` task
3. ✅ All workspace crates have Makefile.toml
4. ✅ `workers/rust/Makefile.toml` exists
5. ✅ `cargo-make/` directory with shared templates
6. ✅ All shell scripts extracted to `cargo-make/scripts/`
7. ✅ FFI test delegation for all workers from root
8. ✅ Nextest remains default for Rust tests
9. ✅ CI tasks can use existing `.github/scripts/`
10. ✅ TypeScript multi-runtime FFI testing preserved
11. ✅ Port 8085 documented for TypeScript (not 8084)
12. ✅ `extends` pattern used in all Makefile.toml files
