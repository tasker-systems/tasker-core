...
## Phase 1: Create functional-examples handler mirrors (3 languages in parallel)

### Naming Convention
- Directory: `functional-examples/` alongside existing `examples/`
...
### Python: `workers/python/tests/handlers/functional_examples/`

Mirror directory structure:
```
workers/python/tests/handlers/functional_examples/
...
### TypeScript: `workers/typescript/tests/handlers/functional_examples/`

Mirror structure:
```
workers/typescript/tests/handlers/functional_examples/
...
### Ruby: `workers/ruby/spec/handlers/functional_examples/`

Mirror structure:
```
workers/ruby/spec/handlers/functional_examples/
├── linear_workflow/
...
- Auto-discovery: `workers/ruby/lib/tasker_core/test_environment.rb` (glob pattern `spec/handlers/examples/**/*_handler.rb` — need to add `functional_examples/` pattern)
...
**Python**: Update `workers/python/tests/handlers/functional_examples/__init__.py` to export all DSL handler classes. The Python worker's handler discovery scans `StepHandler` subclasses.

**TypeScript**: Add `ALL_DSL_HANDLERS` array to `functional_examples/index.ts`. Update the test runner to register both `ALL_EXAMPLE_HANDLERS` and `ALL_DSL_HANDLERS`.

**Ruby**: Update `workers/ruby/lib/tasker_core/test_environment.rb` to also glob `spec/handlers/functional_examples/**/*_handler.rb` (or the functional files use a different naming convention — the DSL blocks may need explicit require).
