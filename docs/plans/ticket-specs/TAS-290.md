...
This plan migrates `crates/workers/typescript/` from koffi to napi-rs.
...
## Phase 1: Rust FFI Layer (`crates/workers/typescript/src-rust/`)

Replace C FFI with napi-rs exports. The spike has ~80% of the code; this phase fills gaps.

### 1.1 Update Cargo.toml
**File**: `crates/workers/typescript/Cargo.toml`
...
**File**: `crates/workers/typescript/build.rs`
- Add `napi_build::setup()`
- Keep RUSTC_VERSION capture for `get_rust_version()`

### 1.3 Replace lib.rs
**File**: `crates/workers/typescript/src-rust/lib.rs`
...
**File**: `crates/workers/typescript/src-rust/bridge.rs`
...
**File**: `crates/workers/typescript/src-rust/client_ffi.rs`
...
**File**: `crates/workers/typescript/src-rust/error.rs`
- Copy spike `error.rs` (NapiFfiError → napi::Error)

### 1.7 New ffi_logging.rs
**File**: `crates/workers/typescript/src-rust/ffi_logging.rs`
...
## Phase 2: TypeScript FFI Layer (`crates/workers/typescript/src/ffi/`)
...
- `tools/scripts/ffi-build/build-typescript.sh` → `npx napi build --release --platform --target <triple>`
...
- `rm -rf workers/typescript-napi/` (keeping `docs/plans/ticket-specs/TAS-290.md` as historical reference)

---

## Step 2: Update build scripts

### 2.1 Rewrite `tools/scripts/ffi-build/build-typescript.sh`
**File**: `tools/scripts/ffi-build/build-typescript.sh`
...
### 2.2 Update `tools/scripts/ffi-build/verify-artifacts.sh`
**File**: `tools/scripts/ffi-build/verify-artifacts.sh` (lines 196-213)

Update `verify_typescript()` to look for `.node` files instead of `.so/.dylib`:
- `tasker_ts-${TARGET}.node` (or the napi naming convention)

### 2.3 Update `tools/cargo-make/scripts/ci-restore-typescript-artifacts.sh`
**File**: `tools/cargo-make/scripts/ci-restore-typescript-artifacts.sh`

Replace `.so/.dylib` references with `.node` file handling:
- Look for `tasker_ts.*.node` files instead of `libtasker_ts.{so,dylib}`
- Copy to `crates/workers/typescript/` (package root, where `findBundledNodeModule()` looks)
- Set `TASKER_FFI_MODULE_PATH` instead of `TASKER_FFI_LIBRARY_PATH`

### 2.4 Update `tools/cargo-make/scripts/ci-restore-all-artifacts.sh`
**File**: `tools/cargo-make/scripts/ci-restore-all-artifacts.sh` (line 17)
...
- name: Upload TypeScript artifacts
  ...
  with:
    ...
    path: |
      crates/workers/typescript/dist/
      crates/workers/typescript/tasker_ts.*.node
    retention-days: 1
...
No direct changes needed — this workflow calls `tools/scripts/ffi-build/build-all.sh` which calls `build-typescript.sh`. The artifact naming flows through the script updates in Step 2.
...
- name: Bundle native libraries into package
  run: |
    ...
    try_bundle "ffi-artifacts/typescript/tasker_ts-x86_64-unknown-linux-gnu.node" \
               "crates/workers/typescript/tasker_ts.linux-x64-gnu.node" "linux-x64"
    try_bundle "ffi-artifacts/typescript/tasker_ts-aarch64-apple-darwin.node" \
               "crates/workers/typescript/tasker_ts.darwin-arm64.node" "darwin-arm64"
```

Remove `mkdir -p crates/workers/typescript/native` — `.node` files go directly in package root (matching `package.json` `"files": ["*.node"]`).

Also **remove** the `Setup Node.js` step (lines 446-449) — not needed for napi-rs publishing.

---

## Step 4: Update `crates/workers/typescript/Makefile.toml`
...
### 5.4 `crates/workers/typescript/AGENTS.md`
...
**Local dev**: No changes needed to `Brewfile` or `tools/bin/setup-dev.sh` — the napi build is invoked via `cargo build -p tasker-ts`, not via `npx napi build`. The `npx napi build --platform` script in package.json is a convenience for manual use.
...
1. In `tasker-contrib/examples/bun-app/package.json`, change `"@tasker-systems/tasker": "0.1.4"` to point at local path: `"@tasker-systems/tasker": "file:../../../../tasker-core/crates/workers/typescript"`
...
| **Build scripts** | `tools/scripts/ffi-build/build-typescript.sh`, `tools/scripts/ffi-build/verify-artifacts.sh` |
| **CI restore** | `tools/cargo-make/scripts/ci-restore-typescript-artifacts.sh`, `tools/cargo-make/scripts/ci-restore-all-artifacts.sh` |
| **CI workflows** | `build-workers.yml`, `test-typescript-framework.yml`, `release.yml` |
| **Makefile** | `crates/workers/typescript/Makefile.toml` |
| **Docs** | `docs/workers/typescript.md`, `docs/workers/README.md`, `docs/workers/memory-management.md`, `crates/workers/typescript/AGENTS.md`, `docs/development/best-practices-typescript.md` |
...
This mirrors:
- Python: `depythonize(value)` in `convert_python_completion_to_step_result()` (`crates/workers/python/src/conversions.rs:70`)
- Ruby: `serde_magnus::deserialize(&ruby, value)` in `convert_ruby_completion_to_step_result()` (`crates/workers/ruby/ext/tasker_core/src/conversions.rs:103`)
...
**File**: `crates/workers/typescript/src/subscriber/step-execution-subscriber.ts`
...
**File**: `crates/workers/typescript/src/ffi/types.ts`
- Remove `NapiStepResult` interface (lines 115-124)

**File**: `crates/workers/typescript/src/ffi/ffi-layer.ts`
...
cd crates/workers/typescript && cargo build -p tasker-ts
...
| File | Change |
|------|--------|
| `crates/workers/typescript/src-rust/bridge.rs` | Remove `NapiStepResult`, `convert_napi_result_to_rust()`; update `complete_step_event()` to use `serde_json::Value` + `from_value()` |
| `crates/workers/typescript/src/subscriber/step-execution-subscriber.ts` | Replace `buildNapiStepResult()` / `buildErrorNapiStepResult()` with serde-compatible builders; thread executionTimeMs |
| `crates/workers/typescript/src/ffi/types.ts` | Remove `NapiStepResult` interface |
| `crates/workers/typescript/src/ffi/ffi-layer.ts` | Update `completeStepEvent` signature |
| `crates/workers/typescript/tests/**` | Update any tests using `NapiStepResult` |
