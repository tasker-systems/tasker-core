...
ORCHESTRATION_CONFIG="${ORCHESTRATION_CONFIG:-$(pwd)/config/tasker/generated/orchestration-test.toml}"
WORKER_CONFIG="${WORKER_CONFIG:-$(pwd)/config/tasker/generated/worker-test.toml}"
...
# Scope-aware worker flags (set by CI scope detection)
SKIP_RUBY_WORKER="${SKIP_RUBY_WORKER:-false}"
SKIP_PYTHON_WORKER="${SKIP_PYTHON_WORKER:-false}"
SKIP_TYPESCRIPT_WORKER="${SKIP_TYPESCRIPT_WORKER:-false}"

# Port configuration diagnostic
echo "📋 Port configuration:"
echo "   ORCHESTRATION_PORT=$ORCHESTRATION_PORT"
echo "   WORKER_PORT=$WORKER_PORT"
[[ "$SKIP_RUBY_WORKER" != "true" ]] && echo "   RUBY_WORKER_PORT=$RUBY_WORKER_PORT" || echo "   RUBY_WORKER_PORT=$RUBY_WORKER_PORT (SKIPPED)"
[[ "$SKIP_PYTHON_WORKER" != "true" ]] && echo "   PYTHON_WORKER_PORT=$PYTHON_WORKER_PORT" || echo "   PYTHON_WORKER_PORT=$PYTHON_WORKER_PORT (SKIPPED)"
[[ "$SKIP_TYPESCRIPT_WORKER" != "true" ]] && echo "   TYPESCRIPT_WORKER_PORT=$TYPESCRIPT_WORKER_PORT" || echo "   TYPESCRIPT_WORKER_PORT=$TYPESCRIPT_WORKER_PORT (SKIPPED)"

# Build list of ports to check (only include workers that will be started)
PORTS_TO_CHECK="$ORCHESTRATION_PORT $WORKER_PORT"
[[ "$SKIP_RUBY_WORKER" != "true" ]] && PORTS_TO_CHECK="$PORTS_TO_CHECK $RUBY_WORKER_PORT"
[[ "$SKIP_PYTHON_WORKER" != "true" ]] && PORTS_TO_CHECK="$PORTS_TO_CHECK $PYTHON_WORKER_PORT"
[[ "$SKIP_TYPESCRIPT_WORKER" != "true" ]] && PORTS_TO_CHECK="$PORTS_TO_CHECK $TYPESCRIPT_WORKER_PORT"
...
# 4. Start Ruby FFI worker in background (unless skipped by CI scope)
if [[ "$SKIP_RUBY_WORKER" == "true" ]]; then
  echo "💎 Skipping Ruby FFI worker (not in CI scope)"
  RUBY_WORKER_PID=""
else
...
# 5. Start Python FFI worker in background (unless skipped by CI scope)
if [[ "$SKIP_PYTHON_WORKER" == "true" ]]; then
  echo "🐍 Skipping Python FFI worker (not in CI scope)"
  PYTHON_WORKER_PID=""
else
...
# 6. Start TypeScript FFI worker in background (unless skipped by CI scope)
if [[ "$SKIP_TYPESCRIPT_WORKER" == "true" ]]; then
  echo "📜 Skipping TypeScript FFI worker (not in CI scope)"
  ...
  # Check if port is available before starting TypeScript worker
...
echo "   Checking active worker ports..."
for port in $PORTS_TO_CHECK; do
  echo "   Port $port:"
  lsof -i :$port 2>/dev/null | head -3 || echo "     (nothing bound)"
done

if [[ -n "$TYPESCRIPT_WORKER_PID" ]]; then
  ...
  # Check port status after the delay
...
# Build health check script dynamically based on which workers are active
HEALTH_CHECK_SCRIPT="
...
timeout 60 bash -c "$HEALTH_CHECK_SCRIPT"

# 8. Save PIDs for cleanup
mkdir -p .pids
echo "$ORCHESTRATION_PID" > .pids/orchestration.pid
echo "$WORKER_PID" > .pids/worker.pid
[[ -n "$RUBY_WORKER_PID" ]] && echo "$RUBY_WORKER_PID" > .pids/ruby-worker.pid
[[ -n "$PYTHON_WORKER_PID" ]] && echo "$PYTHON_WORKER_PID" > .pids/python-worker.pid
[[ -n "$TYPESCRIPT_WORKER_PID" ]] && echo "$TYPESCRIPT_WORKER_PID" > .pids/typescript-worker.pid

echo "✅ All requested services started and healthy!"
...
[[ -n "$RUBY_WORKER_PID" ]] && echo "   - Ruby Worker:        http://localhost:$RUBY_WORKER_PORT (PID $RUBY_WORKER_PID)" || echo "   - Ruby Worker:        SKIPPED (not in scope)"
[[ -n "$PYTHON_WORKER_PID" ]] && echo "   - Python Worker:      http://localhost:$PYTHON_WORKER_PORT (PID $PYTHON_WORKER_PID)" || echo "   - Python Worker:      SKIPPED (not in scope)"
[[ -n "$TYPESCRIPT_WORKER_PID" ]] && echo "   - TypeScript Worker:  http://localhost:$TYPESCRIPT_WORKER_PORT (PID $TYPESCRIPT_WORKER_PID)" || echo "   - TypeScript Worker:  SKIPPED (not in scope)"
